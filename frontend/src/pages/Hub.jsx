...
function Ranking() {
  const [items, setItems] = useState([])
  useEffect(() => {
    fetch(`${API}/api/hub/ranking`, { headers: authHeaders() }).then(r => r.json
()} ).then(setItems)
  }, [])
  const medalhas = ['🥇', '🥈', '🥉']
  return (
...
function Usuarios() {
  ...
  async function criarUsuario(e) {
    e.preventDefault()
    setErro('')
    const res = await fetch(`${API}/api/hub/users`, {
      method: 'POST', headers: authHeaders(),
    ...
    <div>
      ...
      {mostrarForm && (
        <form onSubmit={criarUsuario} style={{ background: '#1a1d2e', borderRadius: 12, padding: 20, marginBottom: 20 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div>
              abel style={labelStyle}>Nome</label>
              <input value={form.nome} onChange={e => setForm({ ...form, nome: e.target.value })} required style={inputStyle} />
            </div>
            <div>
              abel style={labelStyle}>Email</label>
              <input type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} required style={inputStyle} />
            </div>
            <div>
              abel style={labelStyle}>Senha</label>
              <input type="password" value={form.senha} onChange={e => setForm({ ...form, senha: e.target.value })} required style={inputStyle} />
            </div>
            <div>
              abel style={labelStyle}>Perfil</label>
