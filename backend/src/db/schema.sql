...
async function initDB() {
  ...
  try {
    await client.query(`
      ...
      CREATE TABLE IF NOT EXISTS pops_visits (
        id SERIAL PRIMARY KEY,
        pop_nome TEXT NOT NULL,
        data DATE NOT NULL,
        equipe TEXT,
        status TEXT DEFAULT 'pendente',
        observacao TEXT,
    ...
    if (exists.rows.length === 0) {
      ...
      await client.query(
        `INSERT INTO users (nome, email, senha, role) VALUES ($1,$2,$3,'admin')`,
        ['Lucas Valle Carvalho', adminEmail, hash]
      );
